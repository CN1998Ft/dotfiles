...
if (Get-Command luajit -ErrorAction SilentlyContinue) {
	Set-Alias -Name lua -Value luajit
}

$touch = "$HOME\scoop\apps\git\current\usr\bin\touch.exe"
...
# Use less for pager, which also applies to the python help()
$less_path = $(where.exe less).Replace("\", "\\")
$env:PAGER=$less_path
